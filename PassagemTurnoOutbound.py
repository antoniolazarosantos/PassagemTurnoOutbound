...
def processarMovimento():
    lista_processos = [
        {"nome": "Picking", "arquivo": "movements_report.csv", "delimitador": ",", "campo_data": "Fecha",
         "campo_quantidade": "Cantidad"},
        {"nome": "Packing", "arquivo": "movements_report.csv", "delimitador": ",", "campo_data": "Fecha",
         "campo_quantidade": "Cantidad"},
        {"nome": "Put to wall", "arquivo": "movements_report.csv", "delimitador": ",", "campo_data": "Fecha",
         "campo_quantidade": "Cantidad"}]
    listaResultado = []
...
def atualizarInformacao(janela):
    janela.statusBar().showMessage("Aguarde atualizando informações...")
    df = processarMovimento()
    modelo = pdm.pandasModel(df)
    janela.tableView.setModel(modelo)
    rowCount = modelo.rowCount() - 1
    janela.tableView.selectRow(rowCount)
    df.to_excel("RelatorioPassagemdeTurno.xlsx")
    atualizarPlanilhaGoogle(df)
    janela.statusBar().showMessage("")
...
class Ui(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # self.actionAtualizar.triggered.connect(actionAtualizar())
